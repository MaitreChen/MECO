...
from utils.io import save_subject_results, format_feature_name, load_subject_list
from engine.trainer_si_cls import TrainerSI


def print_experiment_header(used_modalities, feat_v_name, feat_e_name, feat_c_name):
    print("\n" + "✨" * 20)
    print("🚀 [SI Active Modalities Configuration]")
...
def main():
    parser = argparse.ArgumentParser(description="Subject-Independent Multimodal Emotion Classification")
    parser.add_argument("--config_file", default="configs/emotion_si/full_cls.json", type=str)
    ...
    for s in all_subjects:
        ...
        # Validation: Check if requested modalities exist on disk
        ...
        for sess in ["session1", "session2", "session3"]:
            ...
            # Check if session has all requested features
            ...
            # Extract Label (From the first available active modality)
            labels = sess_v.get(args.label_type) or sess_e.get(args.label_type) or sess_c.get(args.label_type)
            if labels is None: continue

            num_samples = len(labels)

            # Validate alignment lengths across all requested features
            all_feature_lengths = []
            ...
            full_data_dict[args.label_type].extend(labels)
    ...
    run_name = f"EMO-SI-CLS-{format_feature_name(feat_v_name)}-{format_feature_name(feat_e_name)}-{format_feature_name(feat_c_name)}-nc{args.num_classes}-{timestamp}"
    ...
    for fold_id in range(1, 6):
        ...
        # Map Subject IDs to global sample indices
        ...
        save_subject_results(fold_save_dir, f"Fold{fold_id}", best_metrics)
