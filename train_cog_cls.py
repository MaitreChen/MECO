...
def save_cog_results(save_dir, fold_id, best_metrics, task='cls'):
...
def main():
    parser = argparse.ArgumentParser(description="Subject-Independent Cognitive Classification")
    parser.add_argument("--config_file", default="configs/cognition_si/cog_cls.json", type=str)
    parser.add_argument("--task", default="cls", type=str, help="Enforce classification task")
    parser.add_argument("--pair", type=str, choices=["V", "E", "C", "VE", "VC", "EC", "VEC"], default='V')
    ...
    # Print Configuration
    print("\n" + "✨" * 20)
    print(f"🚀 [SI Cognitive Classification (Healthy vs MCI) - Modality: {args.pair}]")
    print("✨" * 20 + "\n")
    ...
    for s in all_subjects:
        ...
        for sess in ["session1", "session2", "session3"]:
            ...
            # Alignment check: reference lengths from the first active modality
            ...
            if any(l != num_samples for l in all_lengths):
                continue

            # Extend global dictionary
    ...
    # 3. Execution
    run_name = f"Cog-CLS-5CV-{args.pair}-nc2-{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for fold_id in range(1, 6):
        ...
        save_cog_results(fold_save_dir, f"Fold{fold_id}", best_metrics, task='cls')
    ...
    if results:
        ...
        final_summary_path = os.path.join(master_log_dir, "Final_SI_COG_CLS_Results.json")
