...
from dataloader.emotion_dataset import MECO_SI_REG_Dataset
...
class TrainerSIReg:
    """
    Trainer for Subject-Independent (Leave-One-Subject-Out / K-Fold CV) Regression tasks.
    """
    ...
    def __init__(self, args, full_data_dict, train_indices, val_indices, label_type, save_dir, val_sub):
        ...
        self.train_dataset = MECO_SI_REG_Dataset(
        ...
        self.val_dataset = MECO_SI_REG_Dataset(
            data_source=full_data_dict, label_type=label_type,
            feat_type_video=self.feat_type_video, feat_type_eeg=self.feat_type_eeg, feat_type_ecg=self.feat_type_ecg,
    ...
    def _log_modality_configuration(self):
        self.logger.info(f"=== SI Fold: {self.val_sub} Configuration (Regression) ===")
    ...
    def run(self):
        ...
        self.logger.info(
            f"Fold Best Val CCC={best_stats['ccc']:.4f}, MAE={best_stats['mae']:.4f}, RMSE={best_stats['rmse']:.4f}")
        return best_stats